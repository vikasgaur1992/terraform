...
resource "azurerm_network_interface" "tfresourcegroup1" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.tfresourcegroup1.id
  }
}
...
resource "azurerm_windows_virtual_machine" "tfresourcegroup1" {
  name                  = var.vm_name
  resource_group_name   = azurerm_resource_group.tfresourcegroup1.name
  location              = var.location
  size                  = var.vm_size
...
output "public_ip" {
  value = azurerm_public_ip.tfresourcegroup1.ip_address
}